...
input_timezone() {
    ...
    then
      echo "Timezone=${input_timezone}" >> ${SCRIPT_DIR}/vas.conf
    else
      input_timezone
    fi
}
...
case $corect_timezone in
  Y|y)
    echo "Timezone=${detected_timezone}" >> ${SCRIPT_DIR}/vars.conf
  ;;
  N|n)
    input_timezone
  ;;
...
do
  echo "KeyboardLayout=$keyboard_layout" >> ${SCRIPT_DIR}/vars.conf
  break
done
